...
public class ReturnDataFirstActivity extends AppCompatActivity
	...
	public void onClick(View v) {
		if(v.getId()==R.id.btnExit){
            ...
            intent.putExtra("data","첫 번째 엑티비티가 넘기는 메시지");
            //되돌아올 때 사용되는 메소드가 startActivityForResult
            //인텐트객체와 함께 request_code를 넘긴다.(사용자가 직접 정의)
            startActivityForResult(intent,SECOND_BUTTON);
        }
	}

	//인텐트를 통해서 액티비티를 호출하고 되돌아오는 경우 메소드를 추가해서 작업
    //onActivityResult 호출한다. - 되돌아와서 마무리해야 하므로
    //onActivityResult 오버라이딩해서 처리할 작업을 구현
